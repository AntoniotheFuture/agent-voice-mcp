...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `engine` | string | 平台默认 | TTS 引擎选择：`"piper"`（跨平台神经网络）/ `"cloud"`（云端 API），或不设置使用平台默认 |
...
| `configPath` | string | - | Piper 引擎专用：模型配置文件路径（.onnx.json） |
| `cloud` | object | - | 云端 TTS 引擎专用：见下方「云端 TTS 引擎」章节 |
| `cloud.provider` | string | - | 云端服务商类型：`"openai"` / `"volcano"` / `"custom"` |
...
支持三种典型的云端 TTS 服务商，通过参数化配置生成调用参数，**无需写代码**。
...
### 2. 签名认证型 (`provider: "volcano"`)

适用于火山引擎/豆包语音合成，使用 Bearer Token 认证（在控制台获取 Token）。
...
| `token` | 访问令牌（控制台获取） | 必填 |
| `appId` | 应用 ID | 必填 |
| `voice` | 音色类型 | `zh_female_qingrun` |
| `cluster` | 服务集群 | `volcano_tts` |
| `timeout` | 超时 (ms) | 30000 |

**可选音色**: `zh_female_qingrun` / `zh_male_qingrun` / `zh_female_tianmei` / `zh_male_chunhou` / `zh_female_xiaoqian` / `zh_male_xiaoyu`

---

### 3. 自定义 HTTP 模板 (`provider: "custom"`)

通用抓取型，通过模板变量 `{{text}}` `{{voice}}` `{{rate}}` `{{volume}}` 拼装请求，适配任意 HTTP API。
...
```json
{
  ...
  "cloud": {
    ...
    "bodyTemplate": "{\"input\":\"{{text}}\",\"voice\":\"{{voice}}\",\"speed\":{{rate}}}",
    "responseType": "binary",
    "timeout": 30000
  },
  "rate": 200,
...
| 字段 | 说明 | 默认值 |
|------|------|--------|
| `url` | 请求 URL（支持模板变量） | 必填 |
| `method` | HTTP 方法 | `POST` |
| `headers` | 请求头（值支持模板变量） | - |
| `bodyTemplate` | 请求体模板 | 必填 |
| `responseType` | 响应类型 | `binary` |
| `responseAudioField` | JSON 响应中的音频字段名 | `data` |
| `timeout` | 超时 (ms) | 30000 |

**模板变量**: `{{text}}`（URL 编码）、`{{rawText}}`（原始文本）、`{{voice}}`、`{{rate}}`、`{{volume}}`

**响应类型说明**:
- `binary` — 响应体直接是音频数据
- `json` — 响应体是 JSON，从 `responseAudioField` 字段取数据（支持 Base64 字符串或音频 URL）

---

## Piper 神经网络引擎

Piper 是开源离线神经网络 TTS 引擎，音质接近 Google TTS 水平，完全离线运行。

### 安装 Piper

```bash
# 使用系统 Python3 安装（macOS 自带）
/usr/bin/python3 -m pip install --user piper-tts
```

### 下载音色模型

从 [Hugging Face Piper Voices](https://huggingface.co/rhasspy/piper-voices) 下载 .onnx 模型和 .json 配置：

```bash
mkdir -p ~/.agent-voice/models
curl -sL "https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/zh/zh_CN/huayan/medium/zh_CN-huayan-medium.onnx" \
  -o ~/.agent-voice/models/zh_CN-huayan-medium.onnx
curl -sL "https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/zh/zh_CN/huayan/medium/zh_CN-huayan-medium.onnx.json" \
  -o ~/.agent-voice/models/zh_CN-huayan-medium.onnx.json
```

### 切换到 Piper 引擎

在 `~/.agent-voice/config.json` 中设置：

```json
{
  "engine": "piper",
  "modelPath": "/Users/<用户名>/.agent-voice/models/zh_CN-huayan-medium.onnx",
  "rate": 200,
  "volume": 0.8
}
```

**注意**：Piper 引擎不支持情感模拟（emotion 参数无效果），但支持 `rate`、`volume` 和多个音色模型切换。
...
│       ├── cloud/          # 云端 TTS Provider 层
│       │   ├── types.ts    # Provider 接口 & 配置类型
│       │   ├── engine.ts   # CloudTTSEngine (TTSEngine 桥接)
│       │   └── providers/
│       │       ├── openai.ts   # OpenAI 兼容 Bearer Token
│       │       ├── volcano.ts  # 火山引擎 HMAC-SHA256 签名
│       │       └── custom.ts   # 自定义 HTTP 模板
│       └── factory.ts      # TTS 引擎工厂
├── tests/
│   ├── index.test.ts      # MCP + say 引擎测试 (19 个)
│   ├── piper.test.ts      # Piper 引擎测试 (5 个)
│   └── cloud.test.ts      # Cloud TTS Provider 测试 (7 个)
├── scripts/
│   └── debug.ts           # 调试脚本
├── .trae/
...
- **云端 TTS 引擎**：支持 3 种典型的云端 TTS 服务商类型（参数化配置，无需写代码）
  - `openai` — OpenAI 兼容型，Bearer Token 认证，支持 `/v1/audio/speech` 接口
  - `volcano` — 火山引擎/豆包型，Bearer Token 认证（控制台获取 Token）
  - `custom` — 自定义 HTTP 模板型，通过 `{{text}}` 等变量拼装请求
- factory.ts 自动根据 `os.platform()` 选择对应引擎（darwin→say, win32→sapi, linux→espeak）
- Piper 引擎提升为跨平台通用选项（`engine: "piper"` 所有平台均可用）
- 配置文件新增 `cloud` 字段和 `cloud.provider` 配置项
- 新增 6 个源文件：windows-sapi.ts, linux-espeak.ts, cloud/types.ts, cloud/engine.ts, providers/openai.ts, providers/volcano.ts, providers/custom.ts
